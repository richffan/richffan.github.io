...
此次审计对被审部门关于差旅费的报销的相关制度、流程的建立健全进行了检查，对内部控制的执行有效性进行了解，对差旅费报销的异常情况进行延伸审计，查看报销审批单及对应的报销原始凭证，汇总分析报销数据，并关联出差申请单，查看原始票据的日期及内容是否与出差行程一致。通过询问、观察、检查、重新计算等审计程序,我们获取了必要的审计依据。
